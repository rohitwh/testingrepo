	...
	public function register(){
		...
		if($this->form_validation->run() == TRUE){			
			...
			if($resmessage=="Inserted successfully"){
				...
				$config['smtp_user'] = "automechanize@gmail.com"; 
				$config['smtp_pass'] = "pramod1234";
				...
				$this->email->from('admin@automechanise.com', 'admin');
				$this->email->to($email);
								
				$this->email->subject('Registration Verification');
				$email_body = '<body style="margin:10px; padding:10px; background-color:#DDECF4;width:700px">
<div class="" style="margin:10px; font-family:Arial, Helvetica, sans-serif; font-size:12px;background-color:#73879E;color:#fff;width:700px">
	...
	public function bookservice($param1=''){
		...
		//$response = $this->rest->response();
		//$servicename = json_decode($response);
		$getcustomername = $this->rest->get('serverapi/Requestquoteapi/datacustomernewrequest/'.$user,'');		
		$response=$this->rest->response();
	...
	public function amentiessearch(){ //vejitha	
	...
	public function askquestion(){	//vejitha	
		//print_r($_POST);exit;
	...
	public function reactiverequest($param1='',$param2=''){//vejitha
		//echo $param2;
	...
	public function completedservices(){//vejitha
		$this->security_model->check_no_customer_login();
		$customer = $this->session->userdata('pkcustomerid');
	...
	public function paymentinfo(){//vejitha
		if(isset($_POST['submit'])){
	...
	public function cancelupcommingappointment($appointmentid){		
		...
		//$token = \Stripe\Token::retrieve("tok_1BCQhaHXQUit0xTHT9raVAEH");
		//Update to cancel the payment and appointment table
		//Get the appointment details
		$appointmentdetail = $this->rest->get('serverapi/appointmentapi/data/'.$appointmentid,'');
		$response=$this->rest->response();
		$appointment=json_decode($response);
		$fkcustomerid = $appointment[0]->fkcustomerid;
		//print_r($appointment);exit();
		//Get the payment id from Appointmentid		
		$paymentdetail = $this->rest->get('serverapi/paymentapi/paymentidbyappointmentid/'.$appointmentid);
		$response=$this->rest->response();
		$payment=json_decode($response);
		//echo $response.'<br />';exit();
				
		$appointmenttime = (isset($appointment[0]->dateofservice) && $appointment[0]->dateofservice != '')?$appointment[0]->dateofservice:'';
		$beforefourhoursappointment = date($appointmenttime, strtotime('-4 hours'));
		$appointment_strtotime = strtotime($beforefourhoursappointment);
		$current_strtotime = strtotime(date('Y-m-d H:i:s'));
		
		//$interval  = abs($current_strtotime - $appointment_strtotime);
		//$interval_minutes   = round($interval / 60);//Convert to minutes
						
		if(isset($payment->error) && $payment->error != ''){
			$this->session->set_flashdata('err_messages', 'Cancellation Failed! Please contact customer support');			
			
		}else{
			$token = (isset($payment[0]->token) && $payment[0]->token !='')?$payment[0]->token:'';
			$paymentid = $payment[0]->pkpaymentid;
			//Process the cancellation on Stripe\Stripe
			//cancellation fee amount - It is declared in constant as CANCELLATION_FEE, amount is multiply with 100(cent) for Stripe
			$cancellation_fee = CANCELLATION_FEE * 100; //Dollar converted to cent
			if($token != ''){
				$cancellation_fee_msg = '';
				try{
					//If Customer cancels before four hours then no cancellation fee will be applied
					if($current_strtotime > $appointment_strtotime){
						$charge = \Stripe\Charge::create(array(
							"amount" => $cancellation_fee,
							"currency" => "cad",
							"description" => "Cancellation charge",
							"metadata" => array("pkpaymentid" => $payment[0]->pkpaymentid,'fkreqid'=>$payment[0]->fkreqid,'paymentcode'=>$payment[0]->paymentcode,'token'=>$token,'paymentid'=>$paymentid),
							"source" => $token,
						));
						//Call spaddpaymentforcancellation
						//Store the cancellation payment/transaction in payment table
						
						$params = array(
						  'fkvendorid' => '',
						  'fkuserid' => '',
						  'transactioncode' => 'cancellappointment',                     
						  'reqdate' => '',
						  'paymentdate' => '',
						  'paymentstatus' => 'paid',
						  'currency' => 'CAD',
						...
						$insert = $this->rest->post('serverapi/Paymentapi/cancelappointment', $params,'');			
						$response=$this->rest->response();
						
						$cancellation_fee_msg = '$'.CANCELLATION_FEE.' - Cancellation fee has been charged from customer';
					}
					
					$cancel_param = array('appointmentid'=>$appointmentid,'fkcustomerid'=>$fkcustomerid);			
					$cancelappointment = $this->rest->put('serverapi/appointmentapi/cancelappointment/'.$appointmentid,$cancel_param,'');//Cancell Appointment				
						
					$paymentid = $payment[0]->pkpaymentid;
					$arr_paymentid = array('paymentid'=>$paymentid);
					$cancelpayment = $this->rest->put('serverapi/paymentapi/cancelpayment/'.$paymentid,$arr_paymentid,'');//Cancel Payment
					//Send email notification to Customer, Vendor and Portal Admin as well.
		
					$customer_name = $this->session->userdata('userfullname');
					$customer_email = $this->session->userdata('useremail');
					$customer_phone = '';
					$MessageHTML = '';
					$MessageHTML .= '<p>This scheduled  appointment has been canceled successfully. Please check the details mentioned below.</p>';
					$MessageHTML .= '<b><b>Customer Details</b><br />'.'Name :'.$customer_name.'<br />Email : '.$customer_email.'<br />Phone:'.$customer_phone.'</p>';
					
					$MessageHTML .= '<p><b>Service Deatails</b><br />'.'</p>';
					
					$MessageHTML .= $cancellation_fee_msg;
					
					//print($MessageHTML);exit();
					$sub = 'Appointment Cancellation by Customer';
					$from = SMTP_ADMIN_EMAIL;
					$tocustomer = $customer_email;
					$email_temp = file_get_contents(base_url() . 'email/CustomerCancelAppointmentTemplate.html');			
					//$email_temp = str_replace("myurl", base_url() . "index.php/login", email_temp);
					$email_temp = str_replace("#MessageHTML#", $MessageHTML, $email_temp);
					$email_temp = str_replace("#User_Name#", $customer_name, $email_temp);
					//$email_temp = str_replace("#Customer_Phone#", $customer_phone, $email_temp);
					$email_temp = str_replace("#BASE_URL#", base_url(), $email_temp);
					$msg = $email_temp;
					
					/********UNCOMMENT THE LINE BELOW TO SEND EMAIL**********/			
					$this->Emailmodel->cancel_appointment($msg,$sub,$tocustomer,$from);//Send email to Customer	
					//$this->Emailmodel->cancel_appointment($msg,$sub,$tovendor,$from);//Send email to Vendor	
					//$this->Emailmodel->cancel_appointment($msg,$sub,$toadmin,$from);//Send email to Admin	
					
					$this->session->set_flashdata('messages', 'Appointment has been cancelled successfully');
					//echo $cancelappointment.'<br />'.$cancelpayment;exit();
				}
				catch (Exception $e) {
					$this->session->set_flashdata('err_messages', 'Cancellation Failed! Please contact customer support');
					//echo $e;exit();
		...
		//Array ( [__ci_last_regenerate] => 1522760831 [pkuserid] => 4 [username] => admin [admin_id] => [admin_name] => )
    ...
    public function cardetails($id)  {
		...
		//print_r($data['servicehistory']);  exit;
	...
	public function locationsearch(){     /* location search on dashbaord*/
		//print_r($_POST);exit();
